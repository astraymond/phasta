...
export class HomeDeliveryPage {
  ...
  createEvent(
    ...
    this.eventProvider.getCost().child(`${eventName}`).once('value', snapshot => {
      ...
      if(order !== null && eventName == 'propane') {
        eventCost = order;
        console.log(eventCost);
      } else {
        if (order != null && eventName == 'pms') {
          eventCost = order * eventQuantity;
          console.log(eventCost);

        } else {
          if (order != null && eventName == 'diesel') {
            eventCost = order * eventQuantity;
            console.log(eventCost);
    ...
    let bgValidator = [eventName, eventQuantity, eventDate, eventTime, eventPlace];
    let errorBag = ['Service', 'Quantity', 'Date', 'Time', 'Address'];
    let fulMsg = ['Please choose service', 'Please Specify quantity', 'Please Specify a date', 'Please specify time', 'Address field empty'];
    let count = 0;
    ...
    if (count == bgValidator.length) {
        if (expectedTime > this.currentDate) {
          ...
          this.eventProvider.connectedRef.once("value", snap => {
            if (snap.val() === true) {
              this.eventProvider.createEvent(eventName, eventQuantity, eventDate, eventTime, eventPlace, eventCost, '')
                .then(newEvent => {
                  this.loading.dismiss().then(() => {
                    this.loading = null;
                    this.navCtrl.pop();
